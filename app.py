...
st.markdown("Advanced ML-based fake review detection with 88% accuracy")

st.sidebar.header("About")
st.sidebar.info(
    "Machine learning classifier trained on 38,000+ labeled reviews "
    "using Logistic Regression, Naive Bayes, and Neural Network models."
)
...
if review_input:
    ...
    with col3:
        if prediction == "GENUINE":
            st.metric("Authenticity", "High")
        else:
            st.metric("Authenticity", "Low")
    ...
    if prediction == "FAKE":
        st.error(
            ...
            "• Extreme positive/negative language\n"
            "• Generic phrases without specifics\n"
            "• Urgency indicators or manipulation tactics"
        )
    elif prediction == "GENUINE":
        st.success(
            "✅ **This review appears to be GENUINE.**\n\n"
            "Characteristics:\n"
            "• Balanced language with pros and cons\n"
            "• Specific measurements and timeframes\n"
            "• Moderate tone with constructive feedback\n"
    ...
    with example_col1:
        ...
        st.code(
            "Good product for the price. Works well but battery only lasts 5 hours. "
            "Setup was easy. Delivery on time. Overall satisfied.",
            language="text"
        )
    ...
    with example_col2:
        ...
        st.code(
            "ABSOLUTELY INCREDIBLE!!! BEST EVER!!! Life changing!!! "
            "Everyone MUST buy NOW!!! Perfect perfect perfect!!!",
            language="text"
        )

st.markdown("---")
st.caption("ML Fake Review Detector | Logistic Regression, Naive Bayes, Neural Network | Accuracy: 88%")